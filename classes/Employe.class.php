...
Class Employe{
	...
	function Inserer(){
		$sql = "INSERT INTO tEmploye VALUES ('{$this->telephone}', '{$this->nom}', '{$this->prenom}') ";
		$res=DB::Sql($sql);
		var_dump($res);
		die;
	}
	
	public static function GetListeEmp(){
		$sql = "SELECT * from tEmploye";
	...
	public static function Connection($telephone) {
		// table tEmploye
		$sql = "SELECT * FROM tEmploye where telephone='$telephone'";
		$res = DB::Sql($sql);
	...
	function Modifier(){
		$sql="UPDATE tEmploye SET nom='{$this->nom}',prenom='{$this->prenom}' WHERE telephone='{$this->telephone}'";
		$res=DB::Sql($sql);	
	}
